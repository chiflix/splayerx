...
class AppXTarget extends _core().Target {
  ...
  getExtensions(executable, displayName) {
    ...
    if (fileAssociations.length) {
      ...
      const sectionName = appName.toLowerCase().replace(/[^-_.a-z0-9]/g, '_');
      extensions += `
            <uap:Extension Category="windows.fileTypeAssociation">
              <uap:FileTypeAssociation Name="${sectionName}">
                <uap:DisplayName>${appName}</uap:DisplayName>
                <uap:SupportedFileTypes>`;

      for (const item of fileAssociations) {
        const exts = (0, _builderUtil().asArray)(item.ext).map(_platformPackager().normalizeExt);
