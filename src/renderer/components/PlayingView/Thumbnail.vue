...
export default {
  ...
  data() {
    return {
      ...
      imageURL: null,
      autoGenerated: false,
      manualGeneratedIndex: 0,
      leastThumnailPerSecond: 1,
      maxThumbnailNumber: 600,
    };
  },
  watch: {
    currentTime(newValue) {
      ...
      if (this.imageMap.get(currentIndex)) {
        ...
        this.manualGeneratedIndex = currentIndex;
      }
    },
  },
  methods: {
    onMetaLoaded() {
      ...
      this.videoInfoInit();
      this.calculateThumbailPace();
      this.thumbnailCanvasInit();
      this.autoGenerated = true;
      this.thumbnailVideo.currentTime = 0;
    },
    calculateThumbailPace() {
      if (!this.videoInfo.duration) {
        this.videoInfo.thumbnailPace = 3;
        this.videoInfo.thumbnailCount = this.maxThumbnailNumber;
      } else if (this.videoInfo.duration < this.maxThumbnailNumber) {
        this.videoInfo.thumbnailPace = 1;
        this.videoInfo.thumbnailCount = parseInt(this.videoInfo.duration, 10);
      } else {
        this.videoInfo.thumbnailPace
        = parseInt(this.videoInfo.duration / this.maxThumbnailNumber, 10);
        this.videoInfo.thumbnailCount = this.maxThumbnailNumber;
      }
    },
    thumbnailCanvasInit() {
    ...
    getCurrentThumbnail(event) {
      ...
      const autoGeneratedIndex = this.videoInfo.currentIndex;
      const pace = this.videoInfo.thumbnailPace;
      let destTime = null;
      let currentIndex = null;
      const { manualGeneratedIndex } = this;
      const { videoWidth, videoHeight } = document.querySelector('video');
      if (this.autoGenerated) {
        destTime = autoGeneratedIndex * pace;
        currentIndex = autoGeneratedIndex;
        console.log(`[Thumbnail]: Siwtched to auto generated index ${autoGeneratedIndex}.`);
      } else {
        destTime = manualGeneratedIndex * pace;
        currentIndex = manualGeneratedIndex;
        console.log(`[Thumbnail]: Siwtched to manual generated index ${manualGeneratedIndex}.`);
      }
