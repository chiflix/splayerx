export function douyuVideoPause(type: string) {
  if (['normal', 'others'].includes(type)) {
    return 'var timer = setInterval(() => {var video = document.querySelector("video");if (video && !video.paused) { video.pause();}var pause = document.querySelector(".pause-c594e8"); if (pause && timer) { clearInterval(timer);timer = null;pause.click(); } }, 100);';
  }
  return 'var timer = setInterval(() => {var video = document.querySelector("video");if (video && !video.paused) { video.pause();}var pause = document.querySelector(".pause-81a5c3"); if (pause && timer) { clearInterval(timer);timer = null;pause.click(); } }, 100);';
}
