...
app.on('ready', () => {
  ipcMain.on('media-info-request', (event, path) => {
    if (existsSync(path)) {
      splayerx.getMediaInfo(path, (info) => {
        if (event.sender.isDestroyed()) return;
        event.reply('media-info-reply', undefined, info);
      });
    } else {
      event.reply('media-info-reply', new Error('File does not exist.'));
    }
  });
  ...
  ipcMain.on('snapshot-request', (
    ...
    if (existsSync(imagePath)) {
      event.reply('snapshot-reply', undefined, imagePath);
      ...
      event.reply('snapshot-reply', new Error('File does not exist.'));
    }
  });

  ipcMain.on('subtitle-request', (event, videoPath, subtitlePath, streamIndex) => {
    if (existsSync(subtitlePath)) {
      event.reply('subtitle-reply', undefined, subtitlePath);
    } else if (existsSync(videoPath)) {
      ...
      event.reply('subtitle-reply', new Error('File does not exist.'));
    }
  });

  ipcMain.on('thumbnail-request', (
    event, videoPath, imagePath, thumbnailWidth, rowThumbnailCount, columnThumbnailCount,
  ) => {
    if (event.sender.isDestroyed()) return;
    if (existsSync(imagePath)) {
      event.reply('thumbnail-reply', undefined, imagePath);
    } else if (existsSync(videoPath)) {
      ...
      event.reply('thumbnail-reply', new Error('File does not exist.'));
    }
  });
});
