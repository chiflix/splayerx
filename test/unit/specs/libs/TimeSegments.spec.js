import { StreamTimeSegments, VideoTimeSegments, SubtitleTimeSegments } from '@/libs/TimeSegments';
...
describe('SubtitleTimeSegments', () => {
  let timeSeg = new SubtitleTimeSegments();
  beforeEach(() => {
    timeSeg = new SubtitleTimeSegments();
    timeSeg.insert(0, 1);
    timeSeg.insert(2, 3);
  });
  ...
  it('should check return true when last segments not changed', () => {
    expect(timeSeg.check(2)).to.equal(false);
    expect(timeSeg.check(2.1)).to.equal(true);
  });
});
